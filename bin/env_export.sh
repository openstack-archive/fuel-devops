...
ExportSnapshotsDefinitions() {
    echo "=== Export definitions of snapshots" >&4
    OLDIFS="${IFS}"
    IFS=","
    for domain in ${domain_names}; do
...
ExportVolumesDefinitions
ExportVolumesContents
ExportSnapshotsDefinitions #"${domain_names}" "${SNAPSHOT_EXPORT_LIST##,}"

echo "=== Export memory state files" >&4
sudo "${baseDir}/snapshots_export.sh" "${LIBVIRT_EXPORT_DIR}" ||
    exit ${SNAPSHOTEXPORT_ERR}
