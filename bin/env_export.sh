...
SNAPSHOT    - Snapshot name to export. All snapshots from the provided one down
              to root snapshot of the branch will be exported. Provided snapshot
              will be set as a current.
              If you need to export several snapshot branches you have to pass
              snapshot names from the branches (one per branch) and separate
              them by commas. In this case all provided snapshots will be tips
              for the exported branches, and the first snapshot in the list will
              be set as a current one.


NOTE: Extra privileges are needed to copy memory state files. The files are
    created as a read-only for root (owner).

NOTE: If the list of snapshot names contains descendants of the first snapshot
    of the list, script execution will be interrupted with an error, because the
...
-p          - Preserve changes made after creation of the snapshots, which are
              the tips of the snapshot branches, i.e. the volume where changes
              are stored is to be exported as well. Without this option volumes
              with such changes won't be exported (only their definitions).
...
-w          - Volume export is implemented via 'virsh vol-download' command.
              The older versions of libvirt has a bug with vol-download
              performance. This option enables workaround for volumes export
              by using 'cp' command. It works if the volume files have read
              permissions for everyone.
-ww         - The same as '-w' option but uses 'sudo cp'. This option should be
              used if the volume files has no read permission.
