...
GetDiskDevicesFromSnapshot() {
    # Get snapshot XML definition, parse it
...
SetCurrentVMState() {
    ...
    for disk in ${diskdevices}; do
        ...
        TIP_VOLUME_IDS="${TIP_VOLUME_IDS},${new_volid}"
    done
    IFS="${OLDIFS}"
    echo "${TIP_VOLUME_IDS}"
}
    ...
    for domain in ${domain_names}; do
        ...
        for SNAPSHOT_NAME in ${SNAPSHOT_NAMES}; do
            ...
            then
                ...
                VOLUME_EXPORT_LIST="${VOLUME_EXPORT_LIST}${CURRENT_VOLUME_IDS}"
            fi
...
ExportVolumesDefinitions() {
    ...
    # content
    touch "${FUELDEVOPS_EXPORT_DIR}/volumes_empty.txt"
    ...
    for volume in ${volume_names}; do
        if [ "${WORKAROUND}" -eq 0 ]; then
            ...
            if [ "${WORKAROUND}" -eq 1 ]; then
                if [ "${DEFS_ONLY:-0}" -eq 1 ] || [ "${DEBUG}" -eq 1 ]; then
                    echo "  cp -f ${volpath} ${LIBVIRT_EXPORT_VOLUMES_DIR}"
                fi
                if [ "${DEFS_ONLY:-0}" -eq 0 ]; then
...
ExportSnapshotsDefinitions() {
    ...
    for domain in ${domain_names}; do
        ...
        for snapshot in ${SNAPSHOT_EXPORT_LIST##,}; do
            ...
            if [ -n "${memstate_file}" ]; then
                if [ "${DEFS_ONLY:-0}" -eq 1 ] || [ "${DEBUG}" -eq 1 ]; then
                    echo "  sudo cp ${memstate_file} ${LIBVIRT_EXPORT_MEMSTATE_DIR}"
                fi
                if [ "${DEFS_ONLY:-0}" -eq 0 ]; then
                    sudo cp "${memstate_file}" "${LIBVIRT_EXPORT_MEMSTATE_DIR}" || \
    ...
    if [ "${DEFS_ONLY:-0}" -eq 1 ] || [ "${DEBUG}" -eq 1 ]; then
        echo "  sudo chmod +r ${LIBVIRT_EXPORT_MEMSTATE_DIR}/*"
    fi
    if [ "${DEFS_ONLY:-0}" -eq 0 ]; then
        sudo chmod +r "${LIBVIRT_EXPORT_MEMSTATE_DIR}"/*
    fi
    IFS="${OLDIFS}"
}
