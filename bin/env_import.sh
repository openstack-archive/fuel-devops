...
DUMP_PATH - Path where copy of exported environment to import is placed.
            The directory must contain exported data from:
            * fuel-devops DB:
            environment.psql, network.psql, domains.psql, interfaces.psql,
            addresses.psql, diskdevices.psql, volumes.psql, volumes_empty.txt
            * libvirt:
            ** definitions
            domain, network, storage pool, volume, network filter
            ** volume binary files
            ** memory state files
            ** snapshot definitions for every domain


NOTE: Extra privileges are needed to copy snapshot definitions and
    restart libvirt daemon with sudo

The following options are available:

-d          - Dry run. Nothing is to be imported. The script will check for
              needed files and display commands to import environment
