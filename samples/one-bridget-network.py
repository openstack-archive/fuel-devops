...
def one(manager):
    environment = manager.environment_create('test_env8_bridged_network')
    ...
    for i in range(0, 5):
        ...
        system_volume = manager.volume_create(
            name='{0}-system'.format(node.name),
            capacity=25000,
            environment=environment)
        ceph_volume = manager.volume_create(
            name='{0}-ceph'.format(node.name),
            capacity=25000,
            environment=environment)
        manager.node_attach_volume(node=node, volume=system_volume)
        manager.node_attach_volume(node=node, volume=ceph_volume)
