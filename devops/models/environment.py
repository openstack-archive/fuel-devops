...
class Environment(BaseModel):
    ...
    def get_address_pool(self, *args, **kwargs):
        return self.addresspool_set.get(*args, **kwargs)

    # NEW
    def get_address_pools(self, *args, **kwargs):
        return self.addresspool_set.filter(*args, **kwargs)
    ...
    def create_environment(cls, full_config):
        ...
        if not template_validator.validate(full_config):
    ...
    def get_networks(self, *args, **kwargs):
        l2_network_devices = []
        for group in self.get_groups():
            l2_network_devices.extend(group.l2networkdevice_set.filter(*args, **kwargs))
