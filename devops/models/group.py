...
class Group(BaseModel):
    ...
    def get_network_pull(self, *args, **kwargs):
        return self.network_pull_set.get(*args, **kwargs)

    def get_network_pulls(self, *args, **kwargs):
        return self.network_pull_set.filter(*args, **kwargs)
    ...
    def add_l2_network_devices(self, l2_network_devices):
        for name, data in l2_network_devices.iteritems():
            params = data.get('params', {})
            address_pool = self.get_address_pool(name=data['address_pool'])
    ...
    def add_nodes(self, nodes):
        for node in nodes:
            interfaces = node['params'].pop('interfaces', [])
            network_configs = node['params'].pop('network_config', [])
            volumes = node['params'].pop('volumes', [])
    ...
    def add_network_pools(self, network_pools):
        for pool_name, address_pool_name in network_pools.iteritems():
            pass
