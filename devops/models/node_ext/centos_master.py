...
class NodeExtension(object):
    ...
    def post_define(self):
        ...
        admin_ip = str(self.node.get_ip_address_by_network_name(
            settings.SSH_CREDENTIALS['admin_network']))
        interface = self.node.get_interface_by_network_name(
            settings.SSH_CREDENTIALS['admin_network'])
        interface_name = str(interface.label)
        user = str(settings.SSH_CREDENTIALS['login'])
        password = str(settings.SSH_CREDENTIALS['password'])
        ...
        dns_ext = str(dns)
        hostname = str(settings.DEFAULT_MASTER_FQDN)
