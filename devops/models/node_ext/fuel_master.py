...
class NodeExtension(object):
    ...
    def start_node_with_boot_menu(self, keys=None,
                                  build_images=False,
                                  master_is_centos7=False,
                                  iso_connect_as='cdrom',
                                  hostname='nailgun.test.domain.local',
                                  admin_net='fuelweb_admin', nat_interface='',
                                  dns='8.8.8.8', iface='enp0s3'):
        if keys is None:
    ...
    def wait_for_ssh(self, admin_net='admin', port=22, timeout=1200):
        self.node.await(network_name=admin_net, timeout=timeout, by_port=port)

    def _get_ssh_with_password(self, admin_net='admin', login='root',
                               password='r00tme'):
        try:
            ssh_client = self.node.remote(network_name=admin_net,
                                          login=login,
                                          password=password)
    ...
    def get_config(self, admin_net='admin', login='root',
                   password='r00tme', fname='/etc/fuel/astute.yaml'):
    ...
    def save_config(self, conf, admin_net='admin', login='root',
                    password='r00tme', fname='/etc/fuel/astute.yaml'):
        ssh_client = self._get_ssh_with_password(admin_net=admin_net,
                                                 login=login,
                                                 password=password)
    ...
    def config_update_router(self, conf, admin_net='admin'):
        master_iface = self.node.interface_by_network_name(admin_net).first()
        router = master_iface.l2_network_device.address_pool.ip_network[1]

        conf['DNS_UPSTREAM'] = router
        conf['ADMIN_NETWORK']['dhcp_gateway'] = router

    def wait_deploy_end(self, puppet_timeout=6000, admin_net='admin',
