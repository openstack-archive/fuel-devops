...
class Node(DriverModel):
        ...
        for disk in self.disk_devices:
            if disk.device == 'disk':
                ...
                while disk_test is not None:
                    ...
                    if back_count > 500:
                        logger.error("More then 500 snapshots in chain")
                        return
    ...
    def snapshot(self, name=None, force=False, description=None,
        ...
        if self.driver.node_snapshot_exists(self, name):
            ...
            return
        if external:
            if self.driver.libvirt_version < 1002012:
                logger.error(
        ...
        if self.has_snapshot(name):
            ...
            if snapshot.get_type == 'external':
                if snapshot.childrens_num == 0:
                    ...
                    # Looking for last snapshot or create new
                    while self.has_snapshot(revert_name):
                        ...
                        if snapshot_revert.childrens_num == 0:
                            ...
                            self.driver.node_revert_snapshot(node=self,
                                                             name=revert_name)
                            create_new = False
                            break
                        else:
        ...
        if self.has_snapshot(name):
            ...
            if snapshot.get_type == 'external':
                if snapshot.childrens_num == 0:
                    ...
                    logger.error(
                        "Cannot delete external snapshots with childrens")
            else:
                self.driver.node_delete_snapshot(node=self, name=name)
