...
class Node(ParamedModel, BaseModel):
    ...
    def pxe_boot_interface_is_eth0(self):
        ...
        :returns: Boolean
        """
        first_net_name = sorted(self.group.environment.get_networks(), key=lambda network: network.id)[0].name
...
#        return self.volume_set.get(*args, **kwargs)
        volumes = self.volume_set.iterator()
        return [volume for volume in volumes for key in kwargs if volume.__getattribute__(key) == kwargs[key]][0]
