...
class Node(DriverModel):
    ...
    def _snapshot_disk_update(self, name):
        ...
        for xml_disk in xml_snapshot_disks:
            if xml_disk.get('snapshot') == 'external':
                ...
                # Set actual node disks to disks from snapshot
                for disk in self.disk_devices:
                    if disk.device == 'disk':
                        if xml_disk.get('name') == disk.target_dev:
                            if snapshot.childrens_num == 0:
                                disk.volume = Volume.objects.get(uuid=vol_file)
                            else:
                ...
                if snapshot.childrens_num == 0:
                    ...
                    while self.has_snapshot(revert_name):
                        ...
                        if snapshot_revert.childrens_num == 0:
                            ...
                            self._snapshot_disk_update(revert_name)
