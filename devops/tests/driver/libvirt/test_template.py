...
class TestLibvirtL2NetworkDevice(LibvirtTestCase):
    ...
    def test_ips(self):
        ...
        assert map(lambda a: a.ip_address, adm_eth0.addresses) == [
            '10.109.0.2',
        ]

        slave1_node = self.env.get_node(name='slave-01')
        slave1_eth0 = slave1_node.interface_set.get(label='eth0')
        assert map(lambda a: a.ip_address, slave1_eth0.addresses) == [
            '10.109.0.3',
        ]
        slave1_eth1 = slave1_node.interface_set.get(label='eth1')
        assert map(lambda a: a.ip_address, slave1_eth1.addresses) == [
            '10.109.2.2',
        ]
        slave1_eth2 = slave1_node.interface_set.get(label='eth2')
        assert map(lambda a: a.ip_address, slave1_eth2.addresses) == [
            '10.109.1.2',
        ]
        slave1_eth3 = slave1_node.interface_set.get(label='eth3')
        assert map(lambda a: a.ip_address, slave1_eth3.addresses) == [
            '10.109.3.2',
        ]
        slave1_eth4 = slave1_node.interface_set.get(label='eth4')
        assert map(lambda a: a.ip_address, slave1_eth4.addresses) == [
            '10.109.4.2',
        ]

        slave2_node = self.env.get_node(name='slave-02')
        slave2_eth0 = slave2_node.interface_set.get(label='eth0')
        assert map(lambda a: a.ip_address, slave2_eth0.addresses) == [
            '10.109.0.4',
        ]
        slave2_eth1 = slave1_node.interface_set.get(label='eth1')
        assert map(lambda a: a.ip_address, slave2_eth1.addresses) == [
            '10.109.2.2',
        ]
        slave2_eth2 = slave1_node.interface_set.get(label='eth2')
        assert map(lambda a: a.ip_address, slave2_eth2.addresses) == [
            '10.109.1.2',
        ]
        slave2_eth3 = slave1_node.interface_set.get(label='eth3')
        assert map(lambda a: a.ip_address, slave2_eth3.addresses) == [
            '10.109.3.2',
        ]
        slave2_eth4 = slave1_node.interface_set.get(label='eth4')
        assert map(lambda a: a.ip_address, slave2_eth4.addresses) == [
            '10.109.4.2',
        ]
    ...
    def test_life_cycle(self):
        ...
        print map(lambda a: a.name(), networks)
