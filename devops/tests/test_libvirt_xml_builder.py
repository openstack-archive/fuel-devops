...
class TestNetworkXml(BaseTestXMLBuilder):
    ...
    def test_net_name_bridge_name(self):
        ...
        self.assertIn(
            ...
            xml)
        self.assertIn(
            '<bridge delay="0" name="dobr{0}" stp="on" />'
            ''.format(self.net.id), xml)

    def test_forward(self):
        self.net.forward = "test_forward"
        xml = self.xml_builder.build_network_xml(self.net)
        self.assertIn(
            '<forward mode="{0}" />'
    ...
    def test_ip_network(self):
        ...
        self.net.tftp_root_dir = 'test_tftp_root_dir'
        xml = self.xml_builder.build_network_xml(self.net)
        str = '''
...
class TestVolumeXml(BaseTestXMLBuilder):
    ...
    def test_backing_store(self):
        self.vol.backing_store = \
            Mock(uuid="volume_uuid", format="backing_store_format")
        xml = self.xml_builder.build_volume_xml(self.vol)
        self.assertIn(
            '''
    <backingStore>
