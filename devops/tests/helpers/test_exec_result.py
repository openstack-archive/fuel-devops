...
class TestExecResult(TestCase):
    ...
    def test_deprecations(self, logger):
        exec_result = ExecResult('test', stdout=[b'{"test": true}'])
        for depr in ('json', 'yaml'):
            self.assertEqual(
                exec_result['stdout_{}'.format(depr)],
                {'test': True}
            )
            logger.assert_has_calls((
                mock.call.warning(
                    'key {key} is deprecated in favor of {new_key}'.format(
                        key='stdout_{}'.format(depr),
                        new_key=depr
                    )
                ),
            ))
            logger.reset_mock()
            exec_result['stdout_{}'.format(depr)] = {'test': False}
            logger.assert_has_calls((
                mock.call.warning(
            ...
            self.assertEqual(exec_result[depr], {'test': True})
            logger.reset_mock()
