...
class TestNtpInitscript(NtpTestCase):
    ...
    def test_init(self):
        ntp_init = ntp.NtpInitscript(self.remote_mock, 'node', '10.1.0.2')
        assert ntp_init.remote is self.remote_mock
        assert ntp_init.node_name == 'node'
        assert repr(ntp_init) == ('NtpInitscript(remote=<SSHClient()>, '
                                  'node_name=node, admin_ip=10.1.0.2)')
        self.remote_mock.check_call.assert_called_once_with(
            'find /etc/init.d/ | grep /etc/init.d/ntp')

    def test_start(self):
        self.remote_mock.check_call.side_effect = (
        ...
        self.remote_mock.check_call.assert_has_calls((
            mock.call('find /etc/init.d/ | grep /etc/init.d/ntp'),
            mock.call('/etc/init.d/ntp start'),
        ))
    ...
    def test_date(self):
        self.remote_mock.execute.return_value = self.make_exec_result(
            'Thu May 26 13:35:43 MSK 2016')

        ntp_init = ntp.NtpInitscript(self.remote_mock, 'node', '10.1.0.2')
        date = ntp_init.date

        self.remote_mock.execute.assert_called_once_with('date')
    ...
    def test_set_actual_time(self):
        ...
        self.remote_mock.execute.assert_has_calls((
            mock.call("sed -i 's/^server 10.1.0.2 .*/server 10.1.0.2 "
                      "minpoll 3 maxpoll 5 iburst/' /etc/ntp.conf"),
...
""")
        ...
        ntp_init = ntp.NtpInitscript(self.remote_mock, 'node', '10.1.0.2')
        assert ntp_init.get_sync_complete() is True
...
""")
        ...
        ntp_init = ntp.NtpInitscript(self.remote_mock, 'node', '10.1.0.2')
        assert ntp_init.get_sync_complete() is False
    ...
    def test_wait_peer(self):
        ...
        self.wait_mock.assert_called_once_with(
            ntp_init.get_sync_complete, interval=8, timeout=600,
...
class TestNtpPacemaker(NtpTestCase):
    ...
    def test_init(self):
        ntp_pcm = ntp.NtpPacemaker(self.remote_mock, 'node', '10.1.0.2')
        assert ntp_pcm.remote is self.remote_mock
        assert ntp_pcm.node_name == 'node'
        assert repr(ntp_pcm) == ('NtpPacemaker(remote=<SSHClient()>, '
                                 'node_name=node, admin_ip=10.1.0.2)')
...
class TestNtpSystemd(NtpTestCase):
    ...
    def test_init(self):
        ntp_sysd = ntp.NtpSystemd(self.remote_mock, 'node', '10.1.0.2')
        assert ntp_sysd.remote is self.remote_mock
        assert ntp_sysd.node_name == 'node'
        assert repr(ntp_sysd) == ('NtpSystemd(remote=<SSHClient()>, '
                                  'node_name=node, admin_ip=10.1.0.2)')
...
class TestNtpChronyd(NtpTestCase):
    ...
    def test_init(self):
        ntp_chrony = ntp.NtpChronyd(self.remote_mock, 'node', '10.1.0.2')
        assert ntp_chrony.remote is self.remote_mock
        assert ntp_chrony.node_name == 'node'
        assert repr(ntp_chrony) == ('NtpChronyd(remote=<SSHClient()>, '
                                    'node_name=node, admin_ip=10.1.0.2)')
...
0 sources with unknown address""")
        ...
        r = ntp_chrony.get_burst_complete()
        self.remote_mock.check_call.assert_called_once_with(
            'chronyc -a activity')
        assert r is True
...
0 sources with unknown address""")
    ...
    def test_set_actual_time(self):
        ...
        self.wait_mock.assert_called_once_with(
            ntp_chrony.get_burst_complete, timeout=600,
            timeout_msg="Failed to set actual time on node 'node'")
