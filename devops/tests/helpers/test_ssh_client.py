...
from devops.helpers.ssh_client import SSHCreds
...
class TestSSHCreds(TestCase):
    def test_default(self):
        creds = SSHCreds(username)
        self.assertEqual(creds.login, username)
        self.assertIsNone(creds.password)
        self.assertIsNone(creds.public_key)
        self.assertEqual(
            str(creds),
            'SSHCreds(login={}, password=None, public_key=None)'.format(
                username)
        )

    def test_all(self):
        public_key = gen_public_key()
        creds = SSHCreds(username, password, public_key)
        self.assertEqual(creds.login, username)
        self.assertEqual(creds.password, password)
        self.assertEqual(creds.public_key, public_key)
...
class TestSSHClient(TestCase):
    ...
    def test_execute(self, execute_async, client, policy, logger):
        ...
        execute_async.assetr_called_once_with(command)
        chan.assert_has_calls((
    ...
    def prepare_execute_through_host(
        ...
        recv = mock.Mock(return_value=stdout)
        recv_stderr = mock.Mock(return_value=stderr)
        recv_exit_status = mock.Mock(return_value=exit_code)

        channel = mock.Mock()
        channel.attach_mock(recv, 'recv')
        channel.attach_mock(recv_stderr, 'recv_stderr')
    ...
    def test_execute_through_host_default(
            self, transp, client, policy, logger):
        target = '10.0.0.2'
        _login = 'cirros'
        _password = 'cubswin:)'
    ...
    def test_execute_through_host_no_auth(
        ...
        result = ssh.execute_through_host(
            target, command, SSHCreds(_login))
        self.assertEqual(result, return_value)
        get_transport.assert_called_once()
        open_channel.assert_called_once()
        transp.assert_called_once_with(intermediate_channel)
    ...
    def test_execute_through_host_key(
        ...
        result = ssh.execute_through_host(
            target, command, SSHCreds(_login, _password, _public_key))
        self.assertEqual(result, return_value)
        get_transport.assert_called_once()
        open_channel.assert_called_once()
        transp.assert_called_once_with(intermediate_channel)
        ...
        transport.assert_has_calls((
            mock.call.start_client(),
            mock.call.auth_publickey(_login, _public_key),
            mock.call.open_session()
        ))
        channel.assert_has_calls((
            mock.call.exec_command('ls ~ '),
    ...
    def prepare_sftp_file_tests(self, client, policy, logger):
        ...
        return ssh, _sftp, lstat

    def test_exists(self, client, policy, logger):
        ssh, _, lstat = self.prepare_sftp_file_tests(client, policy, logger)
        path = '/etc'
    ...
    def test_open(self, client, policy, logger):
        ssh, _sftp, _ = self.prepare_sftp_file_tests(client, policy, logger)
