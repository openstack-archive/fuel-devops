...
class TestSSHAuth(TestCase):
    ...
    def check_memorize(self):
        ...
        self.assertEqual(id(auth00), id(auth01))
        self.assertEqual(id(auth10), id(auth11))
        self.assertNotEqual(id(auth00), id(auth10))
...
class TestSSHClientInit(TestCase):
        ...
        if auth is None:
            if private_keys is None or len(private_keys) == 1:
                logger.assert_has_calls((
                    mock.call.warning(
                        'SSHClient initialization by '
                        'username/password/private_keys '
                ...
                logger.assert_has_calls((
                    mock.call.warning(
                        ...
                        'Please update your code'),
                    mock.call.info(
                        'Multiple private keys has been set, '
                        'using brute-force.')
                ))
        else:
        ...
        if auth is None:
            expected_calls = [
                ...
                _ssh.connect(
                    ...
                    pkey=None if private_keys is None else private_keys[0],
    ...
    def test_init_memorize(
        ...
        host_01 = '127.0.0.1'
        host_02 = '127.0.0.1'
        ssh00 = SSHClient(host=host_01, auth=SSHAuth())
        ssh01 = SSHClient(host=host_01, auth=SSHAuth())
        ssh02 = SSHClient(host=host_01)
        ssh10 = SSHClient(host=host_02, auth=SSHAuth(username=username))
        ssh11 = SSHClient(host=host_02, auth=SSHAuth(username=username))
        ssh12 = SSHClient(host=host_02, username=username)
        self.assertEqual(id(ssh00), id(ssh01))
        self.assertEqual(id(ssh00), id(ssh02))
        self.assertEqual(id(ssh10), id(ssh11))
        self.assertEqual(id(ssh10), id(ssh12))
        self.assertNotEqual(id(ssh00), id(ssh10))
