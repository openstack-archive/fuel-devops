...
class do_timesync(Command):
    ...
    def take_action(self, parsed_args):
        ...
        for name in sorted(cur_time):
            self.log.info("Current time on '{0}' = {1}".format(
                           name, cur_time[name]))
        ...
        for name in sorted(new_time):
            self.log.info("New time on '{0}' = {1}".format(
                           name, new_time[name]))
...
class do_revert_resume(Command):
    ...
    def take_action(self, parsed_args):
        ...
        if not parsed_args.no_timesync:
            ...
            # TODO: How to run command directly?
...
class do_create(Command):
    ...
    def take_action(self, parsed_args):
        ...
        do_slave_add(parsed_args.name,
                     ...
                     force_define=False)
        env.define()
        admin_node.disk_devices.get(
             device='cdrom').volume.upload(parsed_args.iso_path)
