...
def run_ipmi(user, password, remote_host, cmd,
    ...
    if (out is None) or code != 0:
        ...
        print ("rcode ={} or err ={}".format(code, err))
        return None
...
`           user(str)
        ...
        Attributes:
            ...
            prev_level(str) -- the user privileges level. (default 3)
                    values: 1 - CALLBACK, 2 - USER, 3 - OPERATOR
                            4 - ADMINISTRATOR, 5 - OEM, 15 - NO ACCESS
            remote_host(str) -- remote host name. mandatory
            remote_port(int) -- remote port number.
            emote_lan_interface(str) -- the lan interface. (default 'lanplus')
                    values: lan, lanplus
    """

    connection_string = ParamField(default="NA")
    user = ParamField()
    password = ParamField()
    previlegies = ParamField(default='OPERATOR')
    remote_host = ParamField()
    remote_lan_interface = ParamField(default="lanplus")
    remote_port = ParamField(default=623)
    impi_cmd = ParamField(default="ipmitool -I lanplus -L OPERATOR ")
    ...
    @retry()
    def exists(self):
        """  Che1ck if node exists
        Args: None
        Returns: True if successful, False otherwise.
        """
        return Node.check_remote_host(self.user, self.password, self.remote_host)
    ...
    def is_active(self):
        ...
        Returns: True if successful, False otherwise.
        """
        return (Node.power_status(self.user, self.password, self.remote_host) > 0)
    ...
    def send_keys(self,  keys):
        ...
        Note: SOL (Serial over lan) shall be used.
            ...
            will be started and SOL will not be worked until some manipulations
            inside of remote OS. Please take a look at IPMI specification
            at first.
    ...
    def start(self):
       """  Node start
        ...
        Returns: True if successful, False otherwise.
        """
        return Node.power_on(self.user, self.password, self.remote_host)
    ...
    def destroy(self):
       """  Node destroy
        ...
        Returns: True if successful, False otherwise.
        """
        return Node.power_off(self.user, self.password, self.remote_host)
    ...
    def reset(self):
       """  Node reset
        ...
        Returns: True if successful, False otherwise.
        """
        return Node.power_reset(self.user, self.password, self.remote_host)
    ...
    def user_list(user, passw, host):
        ...
        if out.find(Node.get_capabilities().get('UserManagementListReply')) is not None:
    ...
    def power_status(user, passw, host):
        ...
        if out.find(Node.get_capabilities().get('PowerManagementStatus', [])[0]):
            return 1
        elif out.find(Node.get_capabilities().get('PowerManagementStatus', [])[1]):
            return 0
        return None
    ...
    def power_on(user, passw, host):
        ...
        if out.find(Node.get_capabilities().get('PowerManagementOn')) is not None:
            return True
        return False
    ...
    def power_off(user, passw, host):
        ...
        if out.find(Node.get_capabilities().get('PowerManagementOff')) is not None:
            return True
        return False
    ...
    def power_reset(user, passw, host):
        ...
        if out.find(Node.get_capabilities().get('PowerManagementReset')) is not None:
            return True
        return False
    ...
    def power_reboot(user, passw, host):
        ...
        if out.find(Node.get_capabilities().get('PowerManagementCycle')) is not None:
            return True
        return False
    ...
    def chassis_set_boot(user, passw, host, device):
        ...
        if device in Node.get_capabilities().get('ChassisBootManagement'):
            ...
            print out
        if out and out.find(Node.get_capabilities().get('ChassisSetBootDevice')) is not None:
            return True
        return False
