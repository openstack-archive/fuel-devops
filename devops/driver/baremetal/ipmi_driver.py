...
class IpmiNode(Node):
    ...
    def exists(self):
        ...
        :return: bool - True if successful, False otherwise
        """
        return self.conn.check_remote_host(self.name)
    ...
    def is_active(self):
        ...
        :return: bool - True if successful, False otherwise.
        """
        return (0 == self.conn.power_status(self.name))
    ...
    def start(self):
        ...
        self.conn.chassis_set_boot('pxe', self.name)

        if self.is_active():
            # Re-start active node
            res = self.reboot()
        else:
            res = self.conn.power_on(self.name)
    ...
    def destroy(self):
        ...
        :return: bool - True if successful, False otherwise
        """
        res = self.conn.power_off(self.name)
    ...
    def erase(self):
        ...
        if self.is_active():
            return self.conn.power_off(self.name)
        return False
    ...
    def reset(self):
        ...
        :return: bool - True if successful, False otherwise
        """
        return self.conn.power_reset(self.name)
