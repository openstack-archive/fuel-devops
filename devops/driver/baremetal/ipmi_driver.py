...
from devops.models.driver import Driver as IpmiDriver
from devops.models.network import L2NetworkDevice as IpmiL2NetworkDevice
from devops.models.node import Node as IpmiNode
from devops.models.volume import Volume as IpmiVolume
...
def run_ipmi(user, password, remote_host, cmd,
    ...
    lerrors = [(key, value) for (key, value) in impi_cmd_dict.items()
               if value is None]
    if lerrors:
        raise DevopsError('Host:{} Remote:{} ipmitool arguments.\
                           key={}, value={} are not valid'.format(hostname,
                                                                  remote_host,
                                                                  lerrors[0],
                                                                  lerrors[1]))
    ...
    try:
        ...
        raise DevopsError('Host:{} Remote:{} ipmitool command [{}] '
                          ...
                          .format(hostname, remote_host, cmd, message))
