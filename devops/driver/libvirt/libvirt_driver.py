...
class DevopsDriver(object):
    ...
    def node_revert_snapshot(self, node, name=None):
        ...
        if snap_type == 'external' and snap_state == 'paused':
            print "Revert external %s %s" % (node.name, snap_state)
            self.conn.restore(snap_memory.get('file'))
            print "Create work snapshot"
#            self.node_create_snapshot(node, name='revert1', disk_only=True)
            self.node_create_snapshot(node, name='%s-revert' % name)
    ...
    def node_delete_snapshot(self, node, name=None):
        ...
        snap_type = self._get_snapshot_type(snapshot)
        #print snapshot.getXMLDesc()
        # TODO: add update of actual vm, to use parent snapshot/base file
        #       revert system to parent/base snapshot?
        ...
        if snap_type == 'external':
            ...
            if domain.isActive():
                log.error("Cannot delete external snapshot on active domain")
                return
