...
class DevopsDriver(object):
    ...
    def __init__(self,
        ...
        libvirt.registerErrorHandler(self.__libvirt_error_handler, self)
    ...
    def conn(self):
        ...
        return libvirt.open(self.connection_string)

    def __libvirt_error_handler(self, error):
        # this handler mutes libvirt messages to stdout
        pass
    ...
    def node_define(self, node):
        ...
        node_xml = self.xml_builder.build_node_xml(
            node, emulator, numa, if_prefix='fuelnet')
    ...
    def node_destroy(self, node):
        ...
        try:
            ...
            if e.get_error_code() == libvirt.VIR_ERR_SYSTEM_ERROR:
                logger.error(
                    "Error appeared while destroying the domain {}, ignoring"
                    .format(self.conn.lookupByUUIDString(node.uuid).name()))
                return None
            else:
                raise
