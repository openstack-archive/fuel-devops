...
class DevopsDriver(object):
    ...
    @retry()
    def network_filter_ref_add(self, network, interface):
        net_filter_name = "{}_{}".format(network.environment.name,
                                         network.name)
        iface_filter_name = "{}_{}".format(net_filter_name,
                                           interface.mac_address)
        filter_xml = self.conn.nwfilterLookupByName(net_filter_name).XMLDesc()
        filter_xml = ET.fromstring(filter_xml)

        xpath = './filterref[@filter="{}"]'.format(iface_filter_name)
        if not filter_xml.find(xpath):
            filter_xml.append(ET.Element('filterref',
                                         {'filter': iface_filter_name}))
            return self.conn.nwfilterDefineXML(ET.tostring(filter_xml))

    @retry()
    def network_filter_ref_remove(self, network, interface):
        """Remove reference to interface filter from network filter"""
        net_filter_name = "{}_{}".format(network.environment.name,
                                         network.name)
        iface_filter_name = "{}_{}".format(net_filter_name,
                                           interface.mac_address)
        ...
        xpath = './filterref[@filter="{}"]'.format(iface_filter_name)
        if filter_xml.find(xpath):
            filter_xml.remove(filter_xml.find(xpath))
            return self.conn.nwfilterDefineXML(ET.tostring(filter_xml))
    ...
    def network_block_status(self, network):
        ...
        filter_xml = ET.fromstring(filter_xml)
        return filter_xml.find('./rule').get('action') == 'drop'
    ...
    def network_block(self, network):
        ...
        filter_xml = self.conn.nwfilterLookupByName(
            "{}_{}".format(network.environment.name, network.name)).XMLDesc()
        filter_xml = ET.fromstring(filter_xml)
        filter_xml.find('./rule').set('action', 'drop')
        return self.conn.nwfilterDefineXML(ET.tostring(filter_xml))
    ...
    def network_unblock(self, network):
        ...
        filter_xml = self.conn.nwfilterLookupByName(
            "{}_{}".format(network.environment.name, network.name)).XMLDesc()
        filter_xml = ET.fromstring(filter_xml)
        filter_xml.find('./rule').set('action', 'accept')
        return self.conn.nwfilterDefineXML(ET.tostring(filter_xml))
    ...
    def interface_block(self, interface):
        ...
        filter_xml = ET.fromstring(filter_xml)
        for rule in filter_xml.findall('./rule'):
