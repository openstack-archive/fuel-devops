...
class LibvirtDriver(Driver):
    ...
    def get_model_class(self, class_name, subtype=None):
        if class_name == 'L2_Network_Device':
            return LibvirtL2_Network_Device
...
class LibvirtL2_Network_Device(L2_Network_Device):
...
class LibvirtNode(Node):
    ...
    def exists(self, node):
        ...
        """
        try:
            self.driver.conn.lookupByUUIDString(node.uuid)
            return True
        except libvirt.libvirtError as e:
    ...
    # LEGACY (fuel-qa requires), TO REMOVE
