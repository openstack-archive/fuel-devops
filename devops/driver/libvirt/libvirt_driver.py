...
#    child_ifaces = ParamField(default=[])                 # ['em2', 'ens4p0', ...]
#    child_vlan_ifaces = ParamField(default=[])  # [{'admin': 100},{'admin': 101}, ...]
    vlan_ifaces = ParamField(default=[])        # [100, 101, 102, 103, ...]
    ...
    def create(self, verbose=False):
        ...
        if self.parent_iface.phys_dev is not None:
            #TODO(ddmitriev): check that phys_dev is not the device that is used for default route
            parent_name = self.parent_iface.phys_dev

        elif self.parent_iface.l2_net_dev is not None:
            l2_net_dev = self.group.get_l2_network_device(name=self.parent_iface.l2_net_dev)
        ...
        if parent_name is not '':
            if self.parent_iface.tag:
                parent_iface_name = "{0}.{1}".format(parent_name, str(self.parent_iface.tag))
            else:
                parent_iface_name = parent_name
            #TODO(ddmitriev): check if the parent_name is already used in any bridge
            cmd = 'sudo brctl addif {br} {iface}'.format(br=self.bridge_name(), iface=parent_iface_name)
    ...
    def remove(self, verbose=False):
        if verbose or self.uuid:
            if verbose or self.exists():
                ...
                for vlanid in self.vlan_ifaces:
                    iface_name="{}.{}".format(self.bridge_name(), str(vlanid))
