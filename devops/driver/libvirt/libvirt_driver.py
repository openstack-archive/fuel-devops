    ...
    def define(self):
        ...
        if self.backing_store:
            ...
            if self.shared_backing_store_name not in b_pool.listVolumes():
                raise error.DevopsError(
                    "Can't create volume {!r}. shared_backing_store_name "
                    "volume {!r} does not exists.".format(
                        self.name, self.shared_backing_store_name))
...
class LibvirtNode(node.Node):
    ...
    def start_bmc(self):
        ...
        # bmc.start(name)
        self.driver.shell.check_call("vbmc start {}".format(name))
