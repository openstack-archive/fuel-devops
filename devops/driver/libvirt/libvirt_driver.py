...
from devops.models.base import Factory
...
class LibvirtDriver(Driver):
    ...
    def get_model_class(self, class_name, subtype=None):
        if class_name == 'L2NetworkDevice':
            return Factory.get_class('LibvirtL2NetworkDevice')
        elif class_name == 'Volume':
            return Factory.get_class('LibvirtVolume')
        elif class_name == 'Node':
            if subtype == 'admin':
                return Factory.get_class('LibvirtAdminNode')
            elif subtype == 'slave':
                return Factory.get_class('LibvirtSlaveNode')
            else:
                return Factory.get_class('LibvirtSlaveNode')
...
class LibvirtVolume(Volume):
    ...
    def _libvirt_volume(self):
        return self.conn.storageVolLookupByKey(self.uuid)
    ...
    def exists(self):
        ...
        """
        try:
            self.conn.storageVolLookupByKey(self.uuid)
            return True
        except libvirt.libvirtError as e:
...
class LibvirtNode(Node):
    ...
    architecture = ParamField(default='kvm', choices=['x86_64', 'i686'])
    ...
    def suspend(self, verbose=False):
        if verbose or self.self.is_active():
