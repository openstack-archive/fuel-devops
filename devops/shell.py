...
class Shell(object):
    ...
    def do_slave_ip_list(self):
        ...
        for l2dev in self.env.get_env_l2_network_devices():
            if address_pool_name and \
                ...
                continue
            if l2dev.address_pool.name not in slave_ips:
                slave_ips[l2dev.address_pool.name] = []
            for node in self.env.get_nodes():
                if self.params.ip_only:
                    slave_ips[l2dev.address_pool.name].append(
        ...
        for ap, n_ips in slave_ips.iteritems():
    ...
    def get_params(self):
        ...
        address_pool_name.add_argument('--address-pool-name', '-A',
