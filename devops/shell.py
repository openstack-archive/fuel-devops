...
class Shell(object):
    def __init__(self, args):
        ...
        if name and command != 'create':
            self.client.select_env(name=name)
    ...
    def do_list(self):
        ...
        for env_name in sorted(env_names):
            self.client.select_env(env_name)
            env = self.client.env
    ...
    def do_show(self):
        columns = [(node.get_vnc_port(), node.name)
                   for node in sorted(self.env.get_nodes(),
                                      key=lambda x: x.name)]
    ...
    def do_admin_setup(self):
        self.client.admin_setup(boot_from=self.params.boot_from,
