...
class SSHAuth(object):
    ...
    def __new__(cls, username=None, password=None, key=None):
        index = hash((cls, username, password, key))
        try:
            return cls.__cache[index]
        except KeyError:
            obj = cls.__cache[index] = super(SSHAuth, cls).__new__(cls)
            return obj
...
class SSHClient(object):
    ...
    def __new__(
            ...
            auth=None):
        if auth is None and private_keys is not None:
            return super(SSHClient, cls).__new__(cls)
        elif auth is not None:
            idx = hash((cls, host, port, auth))
            if idx in cls.__storage:
                return cls.__storage[idx]
            return super(SSHClient, cls).__new__(cls)
        idx = hash((
            cls,
            host,
    ...
    def __init__(
        ...
        # Memorize with real data
        self.__storage[hash(self)] = self
