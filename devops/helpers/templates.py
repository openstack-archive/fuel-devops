...
def create_devops_config(boot_from,
    ...
    config_nodes = config['devops_settings']['nodes']
    if boot_from == 'cdrom':
    ...
    admin_node = create_admin_config(admin_vcpu=admin_vcpu,
    ...
    for ironic_n in range(1, ironic_nodes_count + 1):
        ...
        ironic_node = {
                         ...
                         'networks': 'ironic', #TODO(ddmitriev): use correct network model
        }
        config_nodes.append(slave_node)

#        if settings.OVS_USE_IMAGE:
#        # Assume that OVS_USE_IMAGE contains path to a cloud image.
#            ovs_name = 'ovs_ubuntu-01'
#            ovs_node = {
#                           'name': ovs_name,
#                           'role': 'ovs_ubuntu',
#                           'vcpu': slave_vcpu,
...
#                           'volumes': [
#                                          {
#                                              'name': ovs_name + '-system',
#                                              'capacity': _get_file_size(settings.OVS_USE_IMAGE),
#                                              'source_image': settings.OVS_USE_IMAGE,
#                                          },
#                                      ],
#                           'networks': 'admin', #TODO(ddmitriev): use correct network model
#                       }
#            config_nodes.append(ovs_node)
